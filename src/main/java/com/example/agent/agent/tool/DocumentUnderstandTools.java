...
public class DocumentUnderstandTools {
    ...
    @Tool(
            ...
            description = "理解文档（尤其是大文件、多页扫描版 PDF）的正确方式。当任务涉及 PDF，"
                    ...
                    + "禁止把文档内容/文本打印到 stdout、用 read-file 读进对话、或把 pdftotext/pdfgrep 的输出直接看进上下文"
                    + "——大文档原文成百上千行会灌爆上下文，且扫描版根本抽不出文字；把文件存到磁盘、栅格化成图片文件是允许的。"
                    + "整份抽取用本工具（传 URL 或本地路径）；小型文档想自由问答，也可先 pdftoppm 成图片再交给 understand_image。"

...
**入库约定**：本目录平时是**部署产物**（各环境的 skill 由运维拷入，不提交，此处默认只保留本说明）。例外：需要跨环境保留的**环境特定 skill**（如 `seal-recognition`，绑定具体接口）可按需提交进版本库。通用的、可复用的示例 skill 模板放 `docs/examples/skills/`（已入库）。

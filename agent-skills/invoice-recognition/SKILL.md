...
description: |
  ...
  提取发票信息/金额/发票号码时使用。识别发票优先用本接口，优于通用图片抽取。
---
...
| `return_ocr_text` | bool | 否 | `false` | 是否返回发票内容（会明显变慢）。**图片输入时 `ocr_text` 恒有值，此开关只对文字版 PDF 生效** |
...
[
  {
    ...
    "confidence": 0.95,
    "invoice_type": 1,
    "ocr_text": "{\"发票号码\": \"2431...\", \"发票代码\": \"...\", \"发票类型\": \"\", \"校验码\": \"\", \"开票日期\": \"2023年02月24日\", \"购货方公司名称\": \"...\", \"销售方公司名称\": \"...\", \"发票金额\": \"73906.88\", \"发票总金额\": \"83514.77\", \"是否抵扣联\": \"否\", \"是否含有国税局印章\": \"是\"}",
    "source_text": null
  }
]
```
...
- `ocr_text`：**是一个 JSON 字符串**，需 `json.loads` 再取字段（不要当对象直接用）
- `corp_image_base64`、`source_text`：恒为 `null`，可忽略

### `ocr_text` 字段 → 常见叫法对照

> 用户口中的"购买方/销方/价税合计/税额"等，要按此表映射到接口字段名，不要猜。
...
r.raise_for_status()
results = r.json()                      # list，最多 1 个元素
ocr = json.loads(results[0]["ocr_text"])  # 再解一层拿到字段
```

## 注意

- **识别发票优先用本接口**，不要用 `extract_image_fields`/`understand_image` 猜发票字段——本接口是专用识别，返回结构化字段，准确率明显更高。
- **`ocr_text` 是 JSON 字符串**，必须再 `json.loads` 一次才能取字段。
- **字段名按上面"别名对照"映射**：购买方=`购货方公司名称`，价税合计=`发票总金额`，销售方=`销售方公司名称`，不要自己造字段名。
- **每张图单独调用**：接口一次只处理一个文件；多图请逐张调用再按图汇总成表。
- **检测不到有效发票是错误不是空结果**：返回 HTTP 400，响应体为纯文本
  （`未检测到有效发票` / `发票信息提取不完整` / `PDF处理失败: ...`）；
  解析前先判断 `r.status_code == 200` 再 `json.loads`，非 200 时直接读响应体文本。
- **PDF 只处理最后两页**：超过两页的发票 PDF 会**静默丢弃前面几页**，需要整本时注意拆分或说明。
- **`return_ocr_text=true` 较慢**（内部走 OCR + 大模型提取）；图片输入无需设置（`ocr_text` 恒有值），文字版 PDF 才需要 `true`。
- 建议 `timeout` 给 120s 以上（批量时注意任务总超时 = 张数 × 单张耗时）。
